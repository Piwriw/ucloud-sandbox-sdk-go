...
type ClientInterface interface {
	...
	PostFreeze(ctx context.Context, params *PostFreezeParams, reqEditors ...RequestEditorFn) (*http.Response, error)
...
//
// Corresponds with POST /freeze (the `PostFreeze` operationId).
func (c *Client) PostFreeze(ctx context.Context, params *PostFreezeParams, reqEditors ...RequestEditorFn) (*http.Response, error) {
	req, err := NewPostFreezeRequest(c.Server, params)
	if err != nil {
		return nil, err
	}
	req = req.WithContext(ctx)
...
func NewPostFreezeRequest(server string, params *PostFreezeParams) (*http.Request, error) {
	var err error
	...
	if params != nil {
		...
		if params.MaxCgroups != nil {
			...
			if queryFrag, err := runtime.StyleParamWithOptions("form", true, "maxCgroups", *params.MaxCgroups, runtime.StyleParamOptions{ParamLocation: runtime.ParamLocationQuery, Type: "integer", Format: ""}); err != nil {
				return nil, err
			} else {
				for _, qp := range strings.Split(queryFrag, "&") {
					rawQueryFragments = append(rawQueryFragments, qp)
		...
		if params.MaxWaitMs != nil {

			if queryFrag, err := runtime.StyleParamWithOptions("form", true, "maxWaitMs", *params.MaxWaitMs, runtime.StyleParamOptions{ParamLocation: runtime.ParamLocationQuery, Type: "integer", Format: "int64"}); err != nil {
				return nil, err
			} else {
				for _, qp := range strings.Split(queryFrag, "&") {
					rawQueryFragments = append(rawQueryFragments, qp)
...
type ClientWithResponsesInterface interface {
	...
	// PostFreezeWithResponse Freeze user/pty cgroups before pause and wait for them to stop. Written directly by envd to avoid Process.Start / shell overhead under load.
	//
	// Writing cgroup.freeze only requests a freeze; the kernel stops each task at its
	// next signal-delivery point. This endpoint therefore polls cgroup.events until the
	// cgroups read back frozen, and returns what it observed, so a caller can tell a
	...
	//
	// Corresponds with POST /freeze (the `PostFreeze` operationId).
	PostFreezeWithResponse(ctx context.Context, params *PostFreezeParams, reqEditors ...RequestEditorFn) (*PostFreezeResponse, error)
...
type PostFreezeResponse struct {
	...
	HTTPResponse *http.Response
	// JSON200 the response for an HTTP 200 `application/json` response
	JSON200 *FreezeResult
	// JSON500 the response for an HTTP 500 `application/json` response
	JSON500 *InternalServerError
}

// GetJSON200 returns the response for an HTTP 200 `application/json` response
func (r PostFreezeResponse) GetJSON200() *FreezeResult {
	return r.JSON200
}
...
//
// Corresponds with POST /freeze (the `PostFreeze` operationId).
func (c *ClientWithResponses) PostFreezeWithResponse(ctx context.Context, params *PostFreezeParams, reqEditors ...RequestEditorFn) (*PostFreezeResponse, error) {
	rsp, err := c.PostFreeze(ctx, params, reqEditors...)
	if err != nil {
		return nil, err
	}
